...
이름 그대로 여러분의 `program`이 활용할 `data`들을 담는 메모리인 것이죠.   
`data`를 제외한 메모리들은 여러분의 `program`이 `run`되면서, "생성되는" 메모리잖아요?   
`data`는 `program`이 `run`되기 전에, 미리 만들어져 있는 메모리입니다.

<br>

왜 `BSS`라는 메모리가 따로 있는지도 알겠나요?

`initialize`되지 않은 `static variable`이 저장되는 `BSS`는,      
...
- [X] 포인터의 연산
  ...
  - `pointer`는 `memory address`를 저장합니다.   
