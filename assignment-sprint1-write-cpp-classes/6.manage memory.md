...
`RET`(`return address`, `return value`를 위한 `temporary variable` 등 함수의 `return`과 관련된 것들)가 저장되는 영역입니다.

`local variable`은 `function` 안에서 `declare`되는 `variable`입니다.   

어떤 `function`이 `subfunction`을 `call`하면

- `subfunction`이 `function`에 `return value`를 전달하기 위한 다양한 정보들(`RET`)
...
- [X] 변수들의 주소 추적
  ...
  `[변수의 메모리 주소] - 변수의 값` 꼴로 출력해보세요.
    ...
    - **여러분의 프로그램에서 `function entry`, `function exit` 시마다 변화한 `stack` 메모리의 그림을 그려보세요.**
      ...
      - `return value`를 위한 `temporary variable`, `main`함수로 돌아가기 위한 `return address` 등으로 인해    
