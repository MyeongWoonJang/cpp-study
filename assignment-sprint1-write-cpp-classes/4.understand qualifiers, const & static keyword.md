...
`pointer to constant`를 `pointer to non-constant`로 바꿔주는 `casting`이죠.   
`pointer`가 가리키는 `data type`의 `const qualification`을 제거합니다.
...
`num`의 값은 언제나 10일 것이니 `num`이 쓰인 곳마다 `num` 대신 10으로 치환해버리는 게 효율적입니다.
...
다른 `memory address`를 가리키는 `data type`이기 때문에,   
사실상 신경써야 하는 `data type`이 두 개이기 때문입니다.
...
많이들 헷갈려 하는 부분입니다.   
`qualifier`가 앞에 붙으면 `referencing value`의 `qualification`을,   
...
`const int* volatile`를 꺼내들 수 밖에 없는 순간이 온 것이죠.   
...
그래서 `pointer`처럼 복잡한 상황에 엮이지 않습니다.   
