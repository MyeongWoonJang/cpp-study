...
`pointer`는 사실상 두 `data type`의 결합이기 때문입니다.
...
- `pointer` 자체에 `qualification`을 부여하는 경우
...
`data member` 중에 `pointer`가 있었을 경우 `pointer` 자체에 `const`가 붙는 경우 등등)   
명시적으로 사용하게 되었다면 그 이유는 반드시 원하는대로 동작하는 `code`를 작성하기 위해서거든요.
...
`member function`에 `qualifier`를 붙이면 특정한 `data type`의 `object`에 대해서만 `call`할 수 있게 됩니다.   
`object`의 `data type`을 `filter`하는 것이죠.

예를 들어, 
- `const member function`은 `object`가 `const`였을 경우에만   
  (즉, `*this`가 `const`였을 경우에만) `call`할 수 있습니다.
- `volatile member function`은 `object`가 `volatile`이었을 경우에만
  (즉, `*this`가 `volatile`이었을 경우에만) `call`할 수 있습니다.
...
`bar`가 `non-const`일 때에는 당연히 `const int& Foo::get() const`가 요구하는   
`qualification`을 만족하지 못하므로 `int& Foo::get()`이 `call`되지만,   
...
`const-qualified member function`과 `volatile-qualified member function`에 이어,  
`ref-qualified member function`이라는 것을 만들 수 있습니다.
...
(`member variable`의 경우 `class`에 대해서 알아야 이해가 쉽습니다.   
지금은 가볍게 읽고 나중에 다시 읽어보세요.)
...
{
    for (int i = 0; i < 5; ++i)
        std::cout << get_increasing_integer() << ' ';
    // output: 1 2 3 4 5
}
```
...
private:
    static int total_widget_cnt = 0;    // <--- here
    ...
}

int Widget::total_widget_cnt;
```
...
public:
    static void update()
    {
        ...
        // 사운드 재생이 종료된 채널을
        // 이용 가능한 채널 목록에 추가합니다.
        std::remove_copy_if(playing_channels.begin(),
            playing_channels.end(),
            [](const Channel& ch){ return ch.done(); })
        ...
    }

    void play()
    {
        // 이용 가능한 채널이 있는지, 사운드가 재생 가능한지 등의
        // 선행 조건을 검사합니다.
        ...

        // 이용 가능한 채널을 재생중인 채널로 옮깁니다.
        playing_channels.push_back(std::move(
            available_channels.top()
        ));
...
private:
    static std::stack<Channel> available_channels;
    static std::list<Channel> playing_channels;
    ...
};
```
