  ...
  - malloc/free와 new/delete의 차이
    ...
    - `C`에서는 `dynamic allocation`을 위해 `malloc`/`free`를 사용하죠.   
      ...
      - `new operator`는 다음의 두 가지 동작을 합니다.
        ...
        - `malloc` (`C`에서의 `malloc` 맞습니다. `new`는 `malloc`을 내부적으로 이용합니다.)
        - `call` `constructor`

      - `delete operator`는 다음의 두 가지 동작을 합니다.

        - `call` `destructor`
        - `free` (`C`에서의 `free` 맞습니다. `delete`는 `free`를 내부적으로 이용합니다.)
      ...
      - `new operator`의 동작에 `operator new`가 포함되어 있는데요.   
      상세한 내용은 다음과 같습니다.
      ...
      **(`delete operator`가 아닌 `operator delete`에 대한 이야기입니다.)**
      ...
      - `delete operator`의 동작에 `operator delete`가 포함되어 있는데요.   
      상세한 내용은 다음과 같습니다.
